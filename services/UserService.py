...
class UserService:
    ...
    def login(self, username=None, password=None, phone=None):
        ...
        Returns:
            tuple: (是否成功, 消息, 用户信息)
        """
        UserService.logger.debug(f"用户登录请求 - 用户名: {username}，手机号: {phone}")
        UserService.logger.info(f"用户登录请求 - 用户名: {username}，手机号: {phone}")
        
        # 参数验证
        if not password:
            UserService.logger.debug("登录失败: 密码不能为空")
            UserService.logger.warning("登录失败: 密码不能为空")
            return False, "密码不能为空", None
        
        if not username and not phone:
            UserService.logger.debug("登录失败: 用户名或手机号不能为空")
            UserService.logger.warning("登录失败: 用户名或手机号不能为空")
            return False, "用户名或手机号不能为空", None
        
        # 密码加密
        encrypted_password = MD5Utils.encrypt(password)
        UserService.logger.debug(f"密码加密完成，加密后: {encrypted_password}")
        UserService.logger.debug("密码加密完成")
        
        # 调用DAO进行登录
        UserService.logger.debug("调用DAO进行登录")
        user = self.user_dao.login_user(username=username, phone=phone, encrypted_password=encrypted_password)
        UserService.logger.debug(f"DAO返回的用户信息: {user}")
        
        if user:
            UserService.logger.debug(f"用户登录成功 - 用户名: {username}，手机号: {phone}")
            UserService.logger.info(f"用户登录成功 - 用户名: {username}，手机号: {phone}")
            
            # 生成token
            user_id = user[0]  # user是一个元组，第一个元素是id
            UserService.logger.debug(f"生成token，user_id: {user_id}, username: {user[1]}, phone: {user[3]}")
            token, token_expiration_time = TokenUtils.generate_token(user_id, user[1], user[3])  # user[1]是username，user[3]是phone
            UserService.logger.debug(f"token生成结果: token={token}, expiration={token_expiration_time}")
            
            if token:
                # 更新token到数据库
                UserService.logger.debug(f"更新token到数据库，user_id: {user_id}, token: {token}, expiration: {token_expiration_time}")
                update_result = self.user_dao.update_token(user_id, token, token_expiration_time)
                UserService.logger.debug(f"token更新结果: {update_result}")
                if update_result:
                    UserService.logger.debug(f"用户token更新成功 - user_id: {user_id}")
                    UserService.logger.info(f"用户token更新成功 - user_id: {user_id}")
                    # 返回用户信息时包含token
                    user_with_token = list(user) + [token, token_expiration_time]
                    UserService.logger.debug(f"返回的用户信息 - user_with_token长度: {len(user_with_token)}")
                    UserService.logger.debug(f"返回的用户信息 - user_with_token内容: {user_with_token}")
                    # 打印每个元素及其类型
                    for i, item in enumerate(user_with_token):
                        UserService.logger.debug(f"user_with_token[{i}]: {item}, 类型: {type(item)}")
                    return True, "login success", user_with_token
                else:
                    UserService.logger.debug(f"用户token更新失败 - user_id: {user_id}")
                    UserService.logger.error(f"用户token更新失败 - user_id: {user_id}")
                    return False, "登录失败: token更新失败", None
            else:
                UserService.logger.debug(f"用户token生成失败 - user_id: {user_id}")
                UserService.logger.error(f"用户token生成失败 - user_id: {user_id}")
                return False, "登录失败: token生成失败", None
        else:
            UserService.logger.debug(f"登录失败: 用户名/手机号或密码错误 - 用户名: {username}，手机号: {phone}")
